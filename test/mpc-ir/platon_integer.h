...
namespace platon
{
	namespace mpc
	{
		...
		public:
			...
			Integer(int32_t length, int64_t value, uint8_t party = PUBLIC);
			Integer(int32_t length,  const void * b);
			...
			int32_t reveal_int32(int party = 0) const;
			
			int64_t reveal_int64(int party = 0) const;
		};
		
		//void init(Bit * bits_, const bool* b, int length, int party = PUBLIC);
	}
}
