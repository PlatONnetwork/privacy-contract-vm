...
void test_Millionaire(int party, T v, T v2) {
	...
	PlatON_RunFuncByLazyJIT(ir_full_name.data(), "free", (char **)result, 0, 0);
...
#ifndef _WIN32
    std::vector<std::string> libs;// { "../lib/libemp-tool.so"};
#else
	std::vector<std::string> libs;//{ "./emp-tool.dll"};
#endif
