...
namespace platon
{
	namespace mpc
	{
		...
		void SysUtils::ms_sleep(unsigned long milisec)
		{
#if defined(WIN32)  || defined(__WIN32__) || defined(_WIN32) || defined(_WIN64) || defined(__WINDOWS__)
			Sleep(milisec);
#else
		...
		char* SysUtils::getErrStr(int error_no)
		{
		...
		uint64_t  SysUtils::getSystemTime(struct timeval *tp, void *tzp)
		{
		...
		uint64_t  SysUtils::getSysCurrSec()
		{
#if defined(WIN32) || defined(_WIN32) || defined(_WIN64)
			time_t clock;
			struct tm tm;
		...
		//获取文件大小
		uint64_t SysUtils::getFileSize(const char* szFile)
		{
			if (!SysUtils::checkFileExists(szFile))
				return -1;
		...
		//获取文件更新数据
		uint64_t SysUtils::getFileUpdateTime(const char* szFile)
		{
			if (!SysUtils::checkFileExists(szFile))
				return -1;
		...
		//检查目录是否存在
		bool SysUtils::checkFileExists(const std::string& filePath)
		{
#if defined(WIN32) || defined(_WIN32) || defined(_WIN64)
		...
		//检查文件是否存在
		bool SysUtils::checkDirExists(const std::string& dirPath)
		{
#if defined(WIN32) || defined(_WIN32) || defined(_WIN64)
		...
		bool SysUtils::createFileDir(const std::string& dirPath)
		{
			if (dirPath.size() <= 0)
				return false;
